...
<title>Circuito Lúdico–Neuromotriz | Cartas + Simulación Grande</title>
...
<style>
  :root{
    ...
    --edu-success:#22c55e;     /* Verde éxito */
  ...
  .station-stage{
    ...
    border-radius:12px; background:#eef6ff; /* más claro y educativo */
...
<body class="min-h-screen">
  ...
  <header class="max-w-6xl mx-auto px-4 pt-6">
    <div class="flex items-center justify-center">
      <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-center">Circuito Lúdico Neuromotriz</h1>
      </div>
    </div>
  </header>
  ...
  <!-- Modal de simulación grande -->
  <div id="sim-modal" class="modal" aria-hidden="true">
    <div class="modal-backdrop"></div>
    <div class="modal-panel p-4 md:p-6">
      <div class="flex items-start justify-between gap-3">
        <div class="flex items-center gap-3">
          <div id="modal-icon" class="text-2xl">🏃‍♂️</div>
          <div>
            <div id="modal-title" class="font-semibold text-lg">Estación</div>
            <div class="text-xs text-slate-600">Simulación por casos</div>
          </div>
        </div>
        <button id="modal-close" class="rounded-lg px-3 py-1.5 bg-slate-100 hover:bg-slate-200 text-slate-800">Cerrar ✕</button>
      </div>

      <div class="mt-4 flex flex-wrap items-center gap-2">
        <button id="btn-success" class="px-3 py-1.5 rounded-md bg-[color:var(--edu-success)] hover:brightness-110 text-white">Éxito</button>
      ...
      <div id="modal-stage" class="station-stage mt-4 grid place-items-center">
        <div class="text-sm text-slate-600">Elige un caso para iniciar la simulación</div>
      </div>

      <div id="modal-msg" class="mt-3 text-sm text-slate-700">
        Selecciona “Éxito” para ver la ejecución fluida o “Dificultad” para ver la situación y la sugerencia.
      </div>
    </div>
  </div>
...
<script>
const stations = [
  { id:1, icon:"🏃‍♂️", name:"Carrera en zigzag", time:60,
    activity:"Correr entre conos colocados en zigzag.",
    issue:"Un estudiante tropieza con un cono.",
    solution:"Puede continuar evitando el cono caído o volver a colocarlo rápidamente; reforzando el cuidado del espacio."
  },
  { id:2, icon:"⚖️", name:"Equilibrio", time:40,
    activity:"Caminar sobre una cuerda o línea recta en el piso.",
    issue:"Se distrae o pierde el equilibrio.",
    solution:"Dar pasos más cortos y extender los brazos como las alas de un avión."
  },
  { id:3, icon:"🦘", name:"Salto de obstáculos", time:60,
    activity:"Saltar dentro de aros en el suelo.",
    issue:"No logra saltar todos.",
    solution:"Ofrecer una variante de caminar rápido por encima, manteniendo la participación."
  },
  { id:4, icon:"🏅", name:"Carrera de relevos con objetos", time:50,
    activity:"Cada niño corre hasta un punto, recoge un objeto y lo lleva hacia otro punto donde se encuentra su compañero.",
    issue:"Un niño se olvida recoger el objeto o lo deja caer.",
    solution:"Recordarle que puede recogerlo rápidamente y seguir, reforzando el trabajo en equipo."
  },
  { id:5, icon:"🎯", name:"Lanzamiento y atrape", time:60,
    activity:"Lanzar pelotas a una caja y que caigan dentro.",
    issue:"No acierta.",
    solution:"Acercar la caja o usar pelotas más grandes para asegurar aciertos."
  },
  { id:6, icon:"👏", name:"Coordinación con palmas", time:45,
    activity:"En parejas realizan una secuencia de palmas; derecha, izquierda, ambas manos al centro.",
    issue:"Dificultad para seguir el ritmo o confusión en la secuencia.",
    solution:"El líder marca el ritmo con palmas fuertes y lentas al inicio; luego aumenta la velocidad gradualmente."
  }
];
  ...
  el.innerHTML = `
    <div class="flex items-start gap-3">
      ...
      <div class="flex-1">
        ...
        <div class="text-sm text-slate-600 mt-0.5"><strong>Solución:</strong> ${s.solution}</div>
      </div>
    </div>
  `;
...
function openModalWithStation(station){
  ...
  modalStage.innerHTML = '<div class="text-sm text-slate-600">Elige un caso para iniciar la simulación</div>';
  modalMsg.textContent = 'Selecciona “Éxito” para ver la ejecución fluida o “Dificultad” para ver la situación y la sugerencia.';
...
function runSimulation(station, stage, msgEl, success = true){
  ...
  // Estación 1: Zigzag con personita
  if(station.id === 1){
    ...
    msgEl.innerHTML = success
      ? "<span style='color:var(--edu-success);font-weight:600'>¡Felicitaciones!</span> Pasó la estación 1 sin problemas."
      : `<span style='color:var(--edu-danger);font-weight:600'>Dificultad: tumbó un cono.</span> ${stations[0].solution}`;
  ...
  // Estación 2: Equilibrio — personita caminando sobre línea
  if(station.id === 2){
    const svgNS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(svgNS,'svg');
    svg.setAttribute('viewBox','0 0 900 260');
    ...
    msgEl.innerHTML = success
      ...
      : `<span style='color:var(--edu-danger);font-weight:600'>Perdió el equilibrio.</span> ${station.solution}`;
    return;
  }

  // Estación 3: Saltos — personita saltando aros
  if(station.id === 3){
    const svgNS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(svgNS,'svg');
    svg.setAttribute('viewBox','0 0 900 260');
    ...
    msgEl.innerHTML = success
      ? "<span style='color:var(--edu-success);font-weight:600'>¡Felicitaciones!</span> Completó los saltos."
      : `<span style='color:var(--edu-danger);font-weight:600'>Le costó un salto.</span> ${station.solution}`;
    return;
  }
        ...
        setTimeout(()=>{
          ...
          if(!success){
            moveTo(300,170, 900, ()=>{
              ...
              msgEl.innerHTML = `<span style='color:var(--edu-danger);font-weight:600'>Dejó caer u olvidó el objeto.</span> ${station.solution}`;
              setTimeout(()=>{
                head.setAttribute('cy', y+3);
                carried.setAttribute('cx', x+10); carried.setAttribute('cy', y+6);
                setTimeout(()=> head.setAttribute('cy', y), 300);
                moveTo(740,200, 1200, ()=>{
                  msgEl.innerHTML += " <span style='color:#0f766e'>¡Listo!</span> Retomó la carrera y entregó al compañero.";
                });
              }, 700);
            });
          } else {
            moveTo(740,200, 1500, ()=>{
              msgEl.innerHTML = "<span style='color:var(--edu-success);font-weight:600'>¡Felicitaciones!</span> Entregó el objeto al compañero y continuaron.";
            });
          }
        }, 300);
      });
  ...
  // Estación 5: Lanzamiento — con lanzador y 3 intentos
  if(station.id === 5){
    const svgNS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(svgNS,'svg');
    svg.setAttribute('viewBox','0 0 900 260');
    ...
    if(success){
      ...
      throwBall({hit:true, delay:1000, index:2});
      setTimeout(()=>{
        msgEl.innerHTML = `<span style='color:var(--edu-danger);font-weight:600'>Uno quedó fuera.</span> ${station.solution}`;
        stage.classList.add('shake'); setTimeout(()=>stage.classList.remove('shake'), 500);
      }, 1800);
    }
    return;
  ...
  // Estación 6: Coordinación con palmas
  if(station.id === 6){
    const svgNS = 'http://www.w3.org/2000/svg';
    const svg = document.createElementNS(svgNS,'svg');
    svg.setAttribute('viewBox','0 0 900 260');
    ...
    function doStep(){
      const s = seq[stepIdx];
      const failing = (!success && cycles % 2 === 0); // alterna fallo/éxito
      if(failing){
